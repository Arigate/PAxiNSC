...
def dTT_from_delta(delta_r, Temp):
    return delta_r / (4.0 + Temp * dgstar(Temp) / gstar(Temp))
...
def axion_background_solver(rho_phi, rho_r, Hub, R, Temp, ma0, ax0, dax0, points_per_cycle):
    ...
    num_hub = 5.0 * rho_phi + 5.0 * rho_r - 3.0 * P_r
    ...
    sol = _integrate_smart_bg(R_arr, damp_arr, mass_sq_pure, R_arr, Y0, int(points_per_cycle))
    R_l, R_osc = find_critical_scales(R_arr, Hub_arr, Temp_arr, ma_arr)

    return sol[0, :], sol[1, :], ma_arr, R_osc, R_l

def axion_pert_coupled_solver(k, rho_phi, rho_r, Hub, Temp, R, delta_r, Phi, dPhi_dR, ma_arr, ax0, dax0, dlnchi_dlnT_arr, points_per_cycle):
    if len(R) < 5: 
    ...
    #Correct implementation of the source term \delta T/T
    dT_T = dTT_from_delta(delta_r, Temp)
    src_dr_arr = np.ascontiguousarray(-dlnchi_dlnT_arr * dT_T, dtype=np.float64)
    
    y0_scalar = float(ax0[0])
    dy0_scalar = float(dax0[0])
    Y0 = np.array([y0_scalar, dy0_scalar, 0.0, 0.0], dtype=np.float64)
    
    sol = _integrate_smart(R_arr, float(k), damp_arr, mode_arr, mass_sq_pure, src_phi_arr, src_dr_arr, phi_arr_c, R_arr, Y0, int(points_per_cycle))
...
def NSC_PERT_axi(rho_phi, rho_r, Hub, Temp, R, k, theta_ini, dtheta_ini, delta_r, Phi, dPhi_dR, ma0, fa_input):
    ...
    ax0_new, dax0_new, ax1, dax1 = axion_pert_coupled_solver(k, rho_phi, rho_r, Hub, Temp, R, delta_r, Phi, dPhi_dR, ma_arr, ax0_ini_arr, dax0_ini_arr, dlnchi_dlnT_arr, points_per_cycle=points_per_cycle)
    ...
    term_source = -dlnchi_dlnT_arr * ma_arr**2 * dT_T * (1.0 - np.cos(ax0_new))
