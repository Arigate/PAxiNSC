...
from NSC_cosmo_V2 import PertEMD_noAX, background
from QCD_axions_V2 import NSC_PERT_axi
...
def noax_mode_solver(k, rho_phi, rho_r, c_r2, omega_r, GG, R):
    delta_r, delta_phi, Phi, theta_phi, theta_r, dPhi_dR = PertEMD_noAX(rho_phi, rho_r, c_r2, omega_r, GG, R, k)
...
def run_parameter_sweep(MA0_array, Tend_array,theta_ini, num_cores):
    ...
    for idx in range(len(Tend_array)):
        ...
        R, rho_phi, rho_r, P_r, Temp, Hub, GG, dT_dR, drho_r_dR, dP_r_dR, R_RH, k_RH  = background(Tend, Tini, maxi, kappa)

        k_values = np.logspace(np.log10(1 * k_RH), np.log10(250 * k_RH), n_k_modes)
        kappas = np.array(k_values) / k_RH
        x = R / R_RH

        num = 16.0 * gstars(Temp) - 12.0 * gstar(Temp) + Temp * (4.0 * dgstars(Temp) - 3.0 * dgstar(Temp))
        den = 4.0 * gstar(Temp) + Temp * dgstar(Temp)
        c_r2 = (1.0 / 3.0) * num / den
        omega_r = (1.0 / 3.0) * (4.0 * gstars(Temp) - 3.0 * gstar(Temp)) / gstar(Temp)
        
        noax_task = partial(noax_mode_solver, rho_phi=rho_phi, rho_r=rho_r, c_r2=c_r2, omega_r=omega_r, GG=GG, R=R)
        ...
        x_end_chi = float(interp1d(Temp[::-1], x[::-1], bounds_error=False, fill_value=np.nan)(0.0101))
        if x_end_chi > 10:
    ...
    master_file_path = os.path.join(root_output_folder, "qcd_mc_extra.npz")
    np.savez_compressed(
        master_file_path,
...
def run_simulations():
    theta_ini=1
    num_cores = max(1, int(cpu_count() * 0.88))
    MA0_array = []
    Tend_array = np.array([
    ...
    MA0_base=3.58*1e-7 *1e-9
    for Tend in Tend_array:
        num = gstar(Tend)**(1/2)
        den = gstars(Tend)**(2/3)
        fac = num / den

        T_rat = (Tend/0.150)**(4/3)

        ang = theta_ini**(4/3)

        MA0_array.append(float(MA0_base * fac * T_rat * ang))
    MA0_array = np.array(MA0_array)

    run_parameter_sweep(MA0_array, Tend_array, theta_ini, num_cores)
